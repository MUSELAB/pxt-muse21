...
namespace Muse_I2C_RS485 {

    let IIC_RS485_ADDR=0x01
     // set dat to reg
    ...
    function ouput(s1: number ,s2:number,s3:number,s4:number,s5:number,s6:number,s7:number,s8:number): number {
        ...
        tb[6] = s7
        tb[7] = s8         
        pins.i2cWriteBuffer(IIC_RS485_ADDR, tb);
        ta = pins.i2cReadBuffer(IIC_RS485_ADDR, 9);
        tc[0] = ta[5]
        tc[1] = ta[6]
        tc[2] = ta[3]
        tc[3] = ta[4]
    ...
    function Before_Correction(): number{
        let tb = pins.createBuffer(8)//Define IIC send data buffer
        let ta = pins.createBuffer(9)//Define IIC accept data buffer
        let tc=[] 
        tb[0] = 0X01
    ...
    function Temperature_and_humidity(): number {
        let tb = pins.createBuffer(8)//Define IIC send data buffer
        let ta = pins.createBuffer(9)//Define IIC accept data buffer
        tb[0] = 0X02
        tb[1] = 0X03
     ...
     */
    //%blockId=Moisture_value
    //% block="Moisture_value %u"
    //% u.defl=Moisture_value
    //export function Moisture_value(): number {
